...
void udp_listen(unsigned int listening_port);
...
// Functions as a server and waits for a UDP message to be received.
// The default listening port is 53.
...
void udp_listen(unsigned int listening_port)
{
  ...
  {
    memset(&buffer,0,64);
    query_thread_params * query_params = (query_thread_params *)malloc(sizeof(query_thread_params));
    if(!query_params)
      continue;
    ...
    //memcpy(query_params->header,&buffer,sizeof(struct DNS_HEADER));
    unsigned short * id = malloc(sizeof(unsigned short));
    memcpy(id,buffer,sizeof(unsigned short));

    
    query_params->sock = sock;
    query_params->len = addr_len; 
    memset(&(query_params->domain),0,256);
    ...
    query_params->addr = malloc(sizeof(struct sockaddr));
...
void ngethostbyname(unsigned char *host , int query_type, int * sock, struct sockaddr_in * c_addr,int len,unsigned short q_id)
{
    ...
    printf("Sending response to the client (id:%x)...",q_id);
    
    memcpy(&buf,&q_id,sizeof(unsigned short)); 

    char * pack = malloc(512);
    memset(pack,0,512);
    memcpy(pack,buf,512);
    memcpy(pack,&q_id,sizeof(unsigned short));
    if( sendto(*sock,pack,512,0,(struct sockaddr *)c_addr,len) < 0)
    {
        perror("sendto failed\n");
    }
    else printf("Done\n");
